...
function getPokemonAvailableGames(pokemon) {
	...
	// -------------------------------------------------
	// Ursprungsspiel ermitteln
	// -------------------------------------------------

	const originKey =
		TRANSFER_GAME_ALIASES[pokemon.game];
	...
	// -------------------------------------------------
	// Pokémon-ID und Form-ID bestimmen
	// -------------------------------------------------
	...
	// -------------------------------------------------
	// Eventuelle historische Transfer-ID
	// -------------------------------------------------

	const evolutionRule =
		POKEMON_TRANSFER_EVOLUTION_RULES[
			currentNumber
		];


	// -------------------------------------------------
	// Erreichbare Stationen bestimmen
	// -------------------------------------------------
	...
	function traverse(currentNode) {
		...
		// ---------------------------------------------
		// Tausch innerhalb derselben Tauschgruppe
		// ---------------------------------------------

		const tradeGroup =
			getTradeGroupForGame(currentNode);
	...
	// -------------------------------------------------
	// Nur echte Spiele prüfen
	// -------------------------------------------------

	const availableGames = [];


	for (const game of reachable) {

		const node =
			TRANSFER_NODES[game];

		if (!node || node.type !== "game") {
			continue;
		}


		// -------------------------------------------------
		// Richtige Pokémon-ID für historische Generationen
		// -------------------------------------------------
		...
		if (
			evolutionRule &&
			evolutionRule.transferAs &&
			evolutionRule.availableFromGeneration
		) {
		...
		const formId =
			pokemonId === currentNumber
				? currentFormId
				: String(pokemonId);


		if (
			isPokemonAvailableInGame(
				pokemonId,
				formId,
				game
			)
